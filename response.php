...
if ($action == 'create_invoice'){
	...
	$invoice_shipping =0; // invoice shipping amount
	$invoice_discount = 0; // invoice discount
	...
	$invoice_gstamount = $_POST['invoice_gstamount'];
	...
	foreach($_POST['invoice_product'] as $key => $value) {
	    ...
	    $invoice_gst = $_POST['invoice_gst'][$key];
	    $invoice_gstamt = $_POST['invoice_gstamt'][$key];
...
// echo $query;exit;
	...
	if($mysqli -> multi_query($query)){
		...
		//Set to
		$invoice->setTo(array($customer_name,$customer_address_1,$customer_address_2,$customer_town,$customer_county,"GST IN: ".$customer_postcode,"Phone: ".$customer_phone));
		//Ship to
		$invoice->shipTo(array($customer_name_ship,$customer_address_1_ship,$customer_address_2_ship,$customer_town_ship,$customer_county_ship,"GST IN: ".$customer_postcode_ship,''));
		//Add items
		// invoice product items
		foreach($_POST['invoice_product'] as $key => $value) {
		    $item_product = $value;
		    ...
		    $invoice->addItem($item_product,'',$item_qty,$invoice_gst,$item_price,$item_subtotal,$item_discount);
		}
		//Add totals
		...
		// }
		$invoice->addTotal("TAX Amount",$invoice_gstamount);
...
// Update product
// Check if the action is 'update_product'
if ($action == 'update_product') {
    ...
    if ($stmt === false) {
        ...
        die(json_encode(array(
            'status' => 'Error',
            'message' => 'Error in preparing SQL statement: ' . $mysqli->error
        )));
    }

    // Bind parameters
    $stmt->bind_param('ssssi', $product_name, $product_gst, $product_desc, $product_price, $getID);
...
if($action == 'add_product') {
    ...
    // Prepare the SQL query
    $query = "INSERT INTO products (product_name, product_gst, product_desc, product_price) VALUES (?, ?, ?, ?)";
    ...
    if($stmt === false) {
        ...
        $stmt->bind_param('siss', $product_name, $product_gst, $product_desc, $product_price);
        if($stmt->execute()) {
            // If execution is successful
            echo json_encode(array(
                'status' => 'Success',
            ...
            echo json_encode(array(
                'status' => 'Error',
                'message' => 'Error in executing SQL statement: ' . $mysqli->error
            ));
        }
    ...
    // If any required POST variables are missing
    echo json_encode(array(
        'status' => 'Error',
        'message' => 'Required POST variables are missing!'
    ));
}
