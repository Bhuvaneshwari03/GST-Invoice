...
// get invoice list
function getInvoices($type = null) {
    ...
    // If 'type' parameter is provided, include it in the WHERE clause
    if ($type) {
        $whereClause = "AND i.status = '$type'";
    }
...
function getInvoices_pending($type = null) {
...
function popProductsList() {
	...
	if($results) {
		...
		while($row = $results->fetch_assoc()) {
		    ...
		    print '<option value="'.$row['product_price'].'" data-gst="'.$row["product_gst"].'">'.$row["product_name"].' - '.$row["product_desc"].'</option>';
		}
		echo '</select>';
